...
document.addEventListener( 'DOMContentLoaded', () => {
	const addExportMenuItem = ( groups, element ) => {
		const actions = {
			name: 'ti_tpc_export',
			title: window.tiTpc.exportLabel,
	...
	const ExportModal = ( { content } ) => {
		...
		const onSave = async () => {
			...
			const url = stringifyUrl( {
				...
				query: {
					...window.tiTpc.params,
					template_name: title || window.tiTpc.textPlaceholder,
		...
		return (
			<Modal title={ window.tiTpc.modalLabel } onRequestClose={ onClose }>
				<TextControl
					label={ window.tiTpc.textLabel }
					placeholder={ window.tiTpc.textPlaceholder }
					value={ title }
					onChange={ setTitle }
				/>
